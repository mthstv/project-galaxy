export const browserLanguage = navigator.language;

export const INITIAL_PLAYER_MOVE_SPEED = 300;
export const INITIAL_PLAYER_SHOOT_SPEED = 0.3;
export const INITIAL_PLAYER_LVL = 1;
export const INITIAL_PLAYER_TOTAL_LIFE = 10;
export const PLAYER_SPECIAL_LIMIT = 6;
export const ASTEROID_LIFE = 12;
export const BULLET_DAMAGE = 4;
export const ASTEROID_DAMAGE = 2;
export const BULLET_SPEED = 750;
export const BACKGROUND_SPEED = 80;
export const ASTEROID_SPEED = 700;
export const PLAYER_MAX_LVL = 10;
export const SPECIAL_METER_GAIN_ON_KILL = 1;
export const BOSS_SPAWN_BACKGROUND_LIMIT = 1;
