  ...
  const canceller = loop(asteroidSpawnRate, () => {
    if (player && player.backgroundProgression < BOSS_SPAWN_BACKGROUND_LIMIT) {
      ...
      const asteroid = add([
        ...
        health(
          Math.round(
            ASTEROID_LIFE * size * ((player.backgroundProgression + 1) / 6)
          )
        ),
        ...
        z(2),
        {
          ...
          moveSpeed: [
            asteroidAngle * -6 * (1 + player.backgroundProgression / 10),
            (ASTEROID_SPEED / size) * (1 + player.backgroundProgression / 10),
          ],
        },
      ]);
